...
arbuthnot$test <- arbuthnot$boys + arbuthnot$girls
```
...
arbuthnot$prob <- arbuthnot$boys/arbuthnot$test
ggplot(data = arbuthnot,aes(x=year,y=prob))+geom_point()+geom_line()
...
arbuthnot$more_boyes <- arbuthnot$boys > arbuthnot$test
```
...
#   mutate(total = ?) %>%
#   arrange(desc(total))
#order() function returns indices
present[c(1:10),c(1,2)]
present<-present[order(-present$total),]
